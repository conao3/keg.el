...
USAGE: keg install [PACKAGE]"
  ...
  (let ((reqinfo (keg-build-get-dependency)))
    (pcase-dolist (`(,pkg . ,_info) (keg-file-read-section 'packages))
      (let ((reqs (keg--alist-get pkg reqinfo)))
        (keg--princ (format " Package: %s" pkg))
...
USAGE: keg info [PACKAGE]"
  ...
  (let ((reqinfo (keg-build-get-dependency))
    ...
    (pcase-dolist (`(,name . ,alist)
                   (if (not pkg)
                       section
                     (list (assoc pkg section))))
      (let ((reqs (keg--alist-get name reqinfo)))
...
USAGE: keg debug"
  ...
  (let ((reqinfo (keg-build-get-dependency)))
    (pcase-dolist (`(,pkg . ,alist) (keg-file-read-section 'packages))
      (let ((reqs (keg--alist-get pkg reqinfo)))
        (keg--princ (format " Package: %s" pkg))
        (keg--princ (format "     Recipe: %s" (keg--alist-get 'recipe alist)))
        (keg--princ (format "     Dependency: %s" reqs))))
